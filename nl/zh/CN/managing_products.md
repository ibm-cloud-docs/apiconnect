...
5. 如果要将产品编译打包到的目录未显示在列表中，请执行以下操作：
    1. 单击**添加和管理目标**。
    2. 单击**添加 IBM Bluemix 目标**。
    3. 选择要发布到的 {{site.data.keyword.Bluemix_short}} **区域**。
    4. 选择要发布到的 {{site.data.keyword.Bluemix_short}} **组织**。
...
- 打开 {{site.data.keyword.Bluemix_short}} **仪表板**。您将在“应用程序”部分中看到应用程序磁贴。
...
您的产品处于“已发布”状态。您的产品已发布到目录中，可供您指定的组织或社区使用。您所选组中的应用程序开发者可以看到并使用该产品内的 API。在包含目录中的“核准”选项卡上，会显示要求使用您产品的所有应用程序开发者请求，您可以在其中拒绝或接受请求。
...
11. 单击电子邮件中的链接以接受邀请。这将打开 {{site.data.keyword.Bluemix_short}} UI。

12. 选择您的 {{site.data.keyword.Bluemix_short}} 组织，然后单击**确认**。
