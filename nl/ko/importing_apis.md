...
1. IBM Integration Toolkit의 REST API 프로젝트와 연관된 Swagger 파일을 {{site.data.keyword.apiconnect_short}} 서비스로 가져오십시오. 
	...
	2. API Manager UI 탐색 분할창에서 **초안** > **API**를 선택하십시오. API 탭이 열립니다. 
...
이 튜토리얼에서는 IBM&reg; App Connect Professional과 {{site.data.keyword.apiconnect_full}}를 사용하여 작성한 REST API를 공개하고 관리할 수 있습니다.

### 전제조건
{: #prereq_pub_api_appconn}

이 튜토리얼을 완료하려면 IBM&reg; App Connect
Professional on Cloud와 IBM API Connect&trade; on
Bluemix&reg;의 올바른 계정이 있어야 합니다. REST
API 파일이 Swagger 버전 2.0 스펙을 준수하는지 확인하십시오. 파일 형식은 JSON 또는 YAML일 수
있습니다. 

IBM&reg; App Connect Professional을 사용하면
RESTful 웹 서비스로서 통합을 노출하는 데 사용될 수 있으며 HTTP 클라이언트에서
호출할 수 있는 특수 애플리케이션인 REST API를 작성할 수 있습니다. API를 작성한 후
...
자세한 이점은 [API 관리](managing_apis.html)를 참조하십시오.
IBM&reg; App Connect Professional에서 REST API를 작성하고
{{site.data.keyword.apiconnect_short}}에 공개하려면
다음 단계를 완료하십시오.

1. IBM&reg; App Connect Professional을 사용하여
REST API를 작성하십시오.
  - IBM ID를 사용하여 [App Connect Professional Web Management Console ![외부 링크 아이콘](../../icons/launch-glyph.svg "외부 링크 아이콘")](https://appconnect.ibmcloud.com/professional/){:new_window}에 로그인하십시오. IBM&reg; App Connect Professional Web Management Console에서 REST API를 작성하는 태스크를 완료하는 방법에 대한 자세한 정보는 IBM&reg; Knowledge Center의 [관리 콘솔 설정 정보 ![외부 링크 아이콘](../../icons/launch-glyph.svg "외부 링크 아이콘")](https://www.ibm.com/support/knowledgecenter/SS3LC4_7.5.2.0/com.ibm.wci.appliance.doc/About_the_WMC/consoleSettings.html){:new_window}를 참조하십시오.
  ...
  <table><caption>표 1. API Connect에 API 공개를 위한 연결 정보</caption>
  <thead>
  <tr>
  <th>필드 이름</th>
  <th>설명</th>
  ...
  <tbody>
  <tr><td>호스트</td>
  <td>관리 클러스터, 서버 또는 클라우드 주소의 호스트 이름을 지정합니다. {{site.data.keyword.Bluemix_short}}의 경우 이 항목은 us.apiconnect.ibmcloud.com일 가능성이 큽니다.</td>
  </tr>
  <tr>
  <td>포트</td>
  ...
  <tr><td>사용자 ID</td>
  <td>관리 클러스터, 서버 또는 클라우드 주소에 액세스하는 데 사용한 인증 사용자 이름을 지정합니다. {{site.data.keyword.Bluemix_short}}에 로그인하는 데 사용하는 IBM ID일 가능성이 큽니다.</td>
  </tr>
  <tr><td>비밀번호</td>
  <td>관리 클러스터, 서버 또는 클라우드 주소에 액세스하는 데 사용한 인증 비밀번호를
  지정합니다. {{site.data.keyword.Bluemix_short}}에 로그인하는 데
...
IBM&reg; App Connect API를 {{site.data.keyword.apiconnect_short}}와 연관시키십시오.

#### Swagger API 정의 가져오기

IBM&reg; App Connect의 REST API 프로젝트와 연관된 Swagger 파일을 {{site.data.keyword.apiconnect_short}} 서비스에 가져오려면 다음 단계를 따르십시오.

1. {{site.data.keyword.apiconnect_short}}Bluemix&reg; 서비스에
로그인하십시오. 

1.  API Manager UI 제목 표시줄에서 **이동 위치** > **초안**을 선택하십시오.
