...
여기서,
    ...
    - `<region>`은(는) {{site.data.keyword.Bluemix_notm}} 지역입니다.

    - `<username_string>`은(는) 기호가 없는 {{site.data.keyword.Bluemix_notm}} 조직 ID입니다. 예를 들어,
...
```
모델이 REST로 노출되면 모든 표준 작성, 읽기, 업데이트 및 삭제 오퍼레이션을 REST 엔드포인트를 통해 사용할 수 있습니다.

10. 선택사항을 입력하고 **Enter**를 누르십시오.
REST에서 모델을 노출하도록 선택한 경우 도구에서
...
사용하십시오.
서버 전용 모델을 작성할지 아니면 서버 또는 클라이언트 LoopBack API 모두에서
...
기본적으로, LoopBack 프로젝트는 개발 및 테스트 용도에 알맞게 인메모리 데이터 소스로 구성되어 제공됩니다. 하지만 언젠가 사용자는 자신의 모델을 실제 모델(예: 데이터베이스 서버)에 연결하고자 할 수 있습니다. 데이터 소스를 추가하려면 다음 단계를 완료하십시오.
...
1. {{site.data.keyword.Bluemix_notm}} UI에서
**컴퓨팅**을 클릭하십시오.
...
1. **선택사항**: 현재 작업 디렉토리가 프로젝트 최상위 레벨 디렉토리인지 확인하십시오. 다음 명령을 입력하십시오.
```
...
apic start
```
그러면 LoopBack 프로젝트(API) 및 Micro Gateway가 로컬로 실행됩니다. 다음 메시지가 표시됩니다.
```
...
브라우저에서 `http://localhost:4001`을 여십시오. 기본 LoopBack(비어 있음 또는 hello-world) 프로젝트의 경우에는 아래와 유사한 내용이 표시됩니다.
```
...
cd <loopback-project-dir>
```
2. 다음 명령을 입력하여 {{site.data.keyword.apiconnect_short}} 및 {{site.data.keyword.Bluemix_notm}}에 로그인하십시오.
```
...
```
여기서,
  * `<region>`은(는) {{site.data.keyword.Bluemix_notm}} 지역입니다. 예:
  * `eu`({{site.data.keyword.Bluemix_notm}} London을 사용 중인 경우)
  * `us`({{site.data.keyword.Bluemix_notm}} Dallas를 사용 중인 경우)
  * `au`({{site.data.keyword.Bluemix_notm}} Sydney를 사용 중인 경우)
...
여기서,
  * `<app>`은(는) 앱의 이름입니다.
  * `<org>`은(는) {{site.data.keyword.Bluemix_notm}} 조직의 이름입니다.
  * `<region>`은(는) {{site.data.keyword.Bluemix_notm}} 지역입니다.
다음과 같은 출력이 리턴됩니다.
  ```
  ...preparing project
...
8. 그런 다음, 공개 시간 중에 리턴된 값으로 제품 정의를 수정해야 합니다. 이를 수행하려면 다음 단계를 완료하십시오.
    ...
    7. 7단계에서 검색한 값으로 다음 필드를 업데이트하십시오.
        - **URL 호출**: API 대상 URL을 삽입하십시오. 보안 프로토콜 HTTPS를 지정해야 합니다. 예:
        ```
...
이 값을 기록해 두지 않은 경우 7단계에서 리턴된 관리 URL로 이동하여 검색할 수 있습니다.
        {{site.data.keyword.Bluemix_notm}}에 로그인한 후 앱 정보를 확인하여 이 값을 검색할 수도 있습니다.
...
4. **추가**를 클릭하십시오. LoopBack**모델** 창이 열립니다.
...
기본적으로, 비어 있는 LoopBack 프로젝트에는 데이터 소스가 정의되어 있지 않습니다. 데이터 소스를 정의하려면 다음 단계를 완료하십시오.
1. **데이터 소스**를 클릭하십시오.

2. **추가** 아이콘을 클릭하십시오.
새로운 LoopBack**모델** 창이 열립니다.
...
npm install --save <connector-package>
```
여기서, `<connector-package>`는 사용자가 선택한 LoopBack 커넥터의 npm 패키지 이름입니다. 커넥터 패키지의 목록은 다음 표를 참조하십시오.
...
LoopBack 프로젝트를 테스트하려면 다음 단계를 완료하십시오.
1. **서버 시작** 아이콘을 클릭하십시오.
다음 메시지가 표시됩니다.
```
http://localhost:<4001/>Running
```
프로젝트 구성 및 기타 프로세스가 실행 중인지 여부에 따라 다른 포트 번호가 표시될 수 있습니다.

2. **localhost:4001**을 클릭하여 API 루트 엔드포인트를 표시하십시오. 기본 LoopBack(비어 있음 또는 hello-world) 프로젝트의 경우에는 아래와 유사한 내용이 표시됩니다.
```
