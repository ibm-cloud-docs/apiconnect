...
서비스를 사용하여 Node.js 및 Java 런타임을 기반으로 API 및 마이크로서비스를 빠르게 작성할 수 있습니다. 이를 작성한 후에는 API를 애플리케이션 개발자와 공유하면서
다양한 레벨의 보안, 가시성, 비용 청구 플랜 및 요금 제한을 설정하여
비즈니스 수준의 제어로 API를 관리할 수 있습니다. 또한 {{site.data.keyword.apiconnect_short}}
...
API를 작성하는 데 필요한 태스크에 대한 자세한 정보는 [API 작성](/docs/services/apiconnect?topic=apiconnect-creating_apis)을 참조하십시오.

## API 관리 개요

제품이 스테이징되고 공개된 이후에는 API Manager를 열어서 보안, 요금 제한, 정책 및 비용 청구 정보를 관리한 후에 해당 제품을 개발자 포털에 공개할 수 있습니다.

다음 다이어그램에 표시된 대로, 제품에는 하나 이상의 API가 포함된 플랜이 들어 있습니다.
...
플랜에서는 제품을 사용하는 고객의 청구 비용도 지정할 수 있습니다. 예를 들어 단일 제품에 3개의 서로 다른 플랜을 정의할 수 있습니다. 각 플랜에는 서로 다른 고객을 대상으로 하는 여러 다른 구독 비용과 여러 다른 요금 제한이 있을 수 있습니다.  
...
사용자는 제품 내에서만 플랜을 작성할 수 있으며, 이 제품은 나중에 카탈로그에서 공개됩니다. 그리고 나서 라이프사이클 관리자는 API Manager를 통해 API 및 플랜의 가용성과 가시성을 제어할 수 있습니다. 개발자 포털을 통해 고객은 API Manager에서 판별된 대로 자신이 사용할 수 있는 플랜 중 하나를 등록할 수 있습니다. 비용 청구가 있는 플랜인 경우 고객이 구독할 때 신용카드 정보를 제공해야 합니다. 사용자는 특정 제품에서 하나의 플랜만 등록할 수 있습니다. 비슷한 목적을 달성할 수 있지만 서로 다른 성능과 비용 레벨을 지닌다는 점에서, 단일 제품 내의 다중 플랜은 유용합니다. 예를 들어, 하나의 API가 사용 가능한 "데모 플랜"과 여러 API가 사용 가능한 "전체 플랜"이 있을 수 있습니다.
...
마지막으로 여러 다른 플랜을 사용하여 청구 비용을 지정할 수 있습니다. 플랜은 무료 플랜 또는 비용 청구가 있는 플랜으로 설정할 수 있습니다. 비용 청구가 있는 플랜을 요금 제한과 함께 사용하여 고객을 위한 여러 다른 서비스 레벨을 설정합니다. 예를 들어, "데모 플랜"은 매월 $5의 비용으로 분당 10개 호출의 요금 제한을 적용할 수 있으며 "전체 플랜"은 매월 분당 $20의 비용으로 최대 1000개 호출까지 허용할 수 있습니다.
...
API를 관리하는 데 필요한 태스크에 대한 자세한 정보는 [API 관리](/docs/services/apiconnect?topic=apiconnect-managing_apis)를 참조하십시오.
...
개발자 포털을 사용하는 데 관한 자세한 정보는 [API 개발 및 사용 ![외부 링크 아이콘](../icons/launch-glyph.svg "외부 링크 아이콘")](https://www.ibm.com/support/knowledgecenter/en/SSFS6T/com.ibm.apic.devportal.doc/capim_devportal_overview.dita){: #new_window}을 참조하십시오.
...
카탈로그를 파티셔닝하려면 [카탈로그 파티셔닝](/docs/services/apiconnect?topic=apiconnect-create_catalog#apic_spaces)을 참조하십시오.
