...
API y microservicios basados en tiempos de ejecución de Node.js y Java. Una vez creados, puede gestionar las API con controles de nivel de negocio estableciendo distintos niveles de seguridad, visibilidad y límites de tarifa, a la vez que comparte las API con desarrolladores de aplicaciones. El servicio de {{site.data.keyword.apiconnect_short}} también le ofrece las herramientas
...
Para obtener más información sobre las tareas necesarias para crear API, consulte [Creación de API](creating_apis.html).

## Visión general de la gestión de API

Después de transferir y publicar un producto, puede abrir API Manager para gestionar la seguridad, los límites de las tarifas y las políticas para después publicar el producto en un portal del desarrollador.

Como se puede ver en el siguiente diagrama, un producto contiene un plan, que a su vez contiene una API.
...
Manager. Mediante el portal del desarrollador, el cliente puede suscribirse a uno de los planes que tiene disponibles, tal como se determina en API Manager. El usuario solo puede suscribirse a un plan de un producto determinado. El hecho de tener varios planes en un solo producto es práctico, puesto que pueden cumplir objetivos similares, pero con distintos niveles de rendimiento. Por ejemplo, tiene un "plan de demostración", que proporciona una sola API, y un "plan completo", que proporciona varias.
...
Para obtener más información sobre las tareas necesarias para gestionar API, consulte [Gestión de API](managing_apis.html).
...
Para particionar un catálogo, consulte [Particionamiento de un catálogo](create_catalog.html#apic_spaces).
