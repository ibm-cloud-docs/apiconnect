...
# Aggiungi una specifica API e richiama un servizio REST con {{site.data.keyword.Bluemix_notm}}
...
Questa esercitazione ti aiuta ad iniziare ad utilizzare velocemente {{site.data.keyword.apiconnect_full}}. Creerai una nuova API in pochi minuti che agisce come un proxy API passthrough per un servizio REST esistente.  L'API che crei fornisce sicurezza e monitoraggio.

## Prerequisito
{: #prereq_tut_add_openapi_rest_bm}

Prima di iniziare, devi [configurare la tua istanza API Connect](/docs/services/apiconnect/tutorials?topic=apiconnect-tut_prereq_set_up_apic_instance).
...
6. Nella finestra *New API*, immetti `New Weather Provider API` per il titolo.
...
10. Nel pannello **Base Path**, nota il valore popolato automaticamente: `/new-weather-provider-api`. Il tuo URL di destinazione dell'API sarà creato da questi valori.  


11. Devi ora definire l'oggetto di risposta restituito quando richiami l'API meteo. A tal scopo, fai clic su **Definitions** nella barra di navigazione.   
...
12. Crea una nuova definizione: **Today**.

13. Aggiungi nuove proprietà per la definizione **Today**.
    - Nome: codice postale         /  Tipo: stringa
...
14. Con i nostri formati dei dati definiti, possiamo creare dei percorsi di richiamo.  Nella barra di navigazione, fai clic su **Paths**. Crea un nuovo percorso facendo clic su **+**.     
    ...
    d. Scorri verso il basso.  Nella sezione **Responses**, modifica **SCHEMA** con _Current_.  
	
	![](images/path-current-responses.png)
	
	e. Salva la tua API.  

15. Ripeti le azioni effettuate nel passo 11 per creare un altro percorso denominato "**/today**".  In questo caso, imposta lo schema **Responses** su _Today_.  Salva la tua API.

16. Salva la tua API.

17. Passa alla scheda **Assemble**. Hai creato due operazioni finora: **GET /current** e **GET /today**. Per assicurarti che venga richiamato l'endpoint corretto, dovrai creare della logica che verrà eseguita in modo condizionale nell'operazione che sta venendo richiamata. Utilizza il costrutto di logica **Operation Switch** per farlo.  **Operation Switch** fornisce un punto di decisione. In base alla coppia verbo/percorso, deve essere richiamata l'operazione appropriata.
    ...
    c. Trascina la politica **invoke** dalla tavolozza e rilasciala nella riga di elaborazione sotto **get /current**. _L'azione di richiamo viene utilizzata per richiamare un servizio esistente dall'interno di un'operazione_.   
    d. Aggiorna il titolo dell'azione con "**invoke-current**".  
	...
	f. Trascina la politica **invoke** dalla tavolozza e rilasciala nella riga di elaborazione sotto **get /today**.  
    g. Aggiorna il suo titolo con "**invoke-today**".  
    h. Aggiorna il campo URL con `https://myweatherprovider.mybluemix.net/today?zipcode=$(request.parameters.zipcode)`.  
	
       ![](images/new-assemble-invoke2.png)
	   

18. Chiudi il pannello di configurazione delle azioni.  Salva la tua API.
...
4. Nel pannello di verifica, seleziona l'operazione **get /current**.
	...
	a. Codice postale è un parametro obbligatorio per questa operazione, per cui immetti un codice postale U.S. valido (ad es. 10504). 
