...
Qui troverai le risposte alle domande per la risoluzione dei problemi comuni relativi all'utilizzo di {{site.data.keyword.apiconnect_long}} su {{site.data.keyword.Bluemix_short}}.
{:shortdesc}

## Nome utente e password obbligatori quando aggiungi il servizio API Connect Bluemix

Dopo aver aggiunto il servizio al tuo dashboard {{site.data.keyword.Bluemix_short}}, ti viene richiesto di inserire un nome utente e una password quando tenti di aprirlo. 

### Sintomi
{: #ts_sym_usernamepw}

Invece di accedere al servizio {{site.data.keyword.Bluemix_short}} direttamente quando apri un nuovo
{{site.data.keyword.apiconnect_short}}, ti viene richiesto di accedere a API Manager.

### Causa 
{: #ts_cause_usernamepw}

Il tuo browser è impostato per bloccare i cookie o il livello di configurazione è a un livello più limitato rispetto a quello richiesto da {{site.data.keyword.apiconnect_short}}.

### Soluzione 
{: #ts_res_usernamepw}

Abilita o aumenta il livello di cookie nelle tue impostazioni del browser finché non viene aperto il servizio {{site.data.keyword.Bluemix_short}}.
...
installare Node o installare i pacchetti a livello globale con npm. Per ulteriori informazioni, vedi [How to Node ![Icona link esterno](../../icons/launch-glyph.svg "Icona link esterno")](http://howtonode.org/introduction-to-npm){: new_window}. **Nota:** la modifica dell'appartenenza della directory è appropriata solo su un sistema di sviluppo locale.
Non effettuare mai questa azione su un sistema di server.
...
{{site.data.keyword.Bluemix_short}} {{site.data.keyword.apiconnect_short}} e che ci siano meno di 25 istanze. Rimuovi un servizio, se hai raggiunto il numero massimo di servizi.
...
Quando utilizzi i forum per fare una domanda, contrassegna con una tag la tua domanda in modo che sia visualizzabile dai team di sviluppo {{site.data.keyword.Bluemix_short}}. 

- Se hai domande tecniche sullo sviluppo o la distribuzione di un'applicazione con {{site.data.keyword.apiconnect_short}},
inserisci la tua domanda in Stack Overflow e contrassegnala con le tag "ibm-bluemix" e "api connect."

- Per domande sul servizio e sulle istruzioni per l'utilizzo iniziale, utilizza il forum
IBM DeveloperWorks dW Answers. Includi le tag "bluemix" e "api connect".
